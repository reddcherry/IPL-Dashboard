...
public class JobCompletionNotificationListener extends JobExecutionListenerSupport {
  ...
  public void afterJob(JobExecution jobExecution) {
    ...
    .stream()
    .forEach(el->{
     Team team = teamData.get((String) el[0]);
     team.setTotalMatches(team.getTotalMatches()+(Long) el[1]);
    });

    entityManager.createQuery("select m.WinningTeam, count(*) from Match m group by m.WinningTeam", Object[].class).getResultList()
    .stream()
    .forEach(e->{
